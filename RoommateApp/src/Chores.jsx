...
import ChoresHistory from "./assets/ChoresHistory";
...
export default function Chores({ householdId, refreshProfile }) {
  ...
  const loadData = useCallback(
    async (silent = false) => {
      ...
      const { data: chores, error: choresError } = await supabase
        .from("chores")
        .select(
          "id, name, due_date, description, household_id, status, repeat_mask, points, image_url",
        )
        .eq("household_id", hid)
        .eq("status", "ongoing");
      ...
      const safeAssignments =
        assignmentsError || !assignments ? [] : assignments;

      const profileIdToName = new Map(
        profiles.map((p) => [
          p.id,
          p.id === user.id ? "You" : p.display_name || "Unnamed",
        ]),
      );
  ...
  async function runOverdueCheck(choresForUI) {
    ...
    console.log(
      `[Chores] marking ${overdueIds.length} overdue chore(s) as passed:`,
      overdueIds,
    );
  ...
  const handleChoreAction = async (chore, meta) => {
    ...
    if (meta?.mode === "abandon") {
      ...
      console.log("[Chores] abandon result:", {
        data: choreData,
        error: choreErr,
      });
      ...
      if (actorProfile) {
        ...
        const currentPoints =
          typeof actorProfile.points === "number" ? actorProfile.points : 0;
        ...
        if (profileUpdateErr) {
          console.error(
            "[Chores] abandon — profile update error:",
            profileUpdateErr,
          );
        }
      }
    ...
    if (meta?.mode === "finish") {
      ...
      console.log("[Chores] finish result:", {
        data: choreData,
        error: choreErr,
      });
      if (choreErr) throw choreErr;
      ...
      if (actorProfile) {
        ...
        const currentPoints =
          typeof actorProfile.points === "number" ? actorProfile.points : 0;
        const currentStreak =
          typeof actorProfile.streaks === "number" ? actorProfile.streaks : 0;
        ...
        if (actorProfile.prev_streak_day) {
          ...
          const diffDays = Math.round(
            (nowMidnight.getTime() - prevMidnight.getTime()) /
              (1000 * 60 * 60 * 24),
          );
        ...
        if (profileUpdateErr) {
          console.error(
            "[Chores] finish — profile update error:",
            profileUpdateErr,
          );
        }
      }
        ...
        onCreate={async (payload) => {
          try {
            if (!resolvedHouseholdId) {
              console.error(
                "[CreateChores] No resolvedHouseholdId; cannot create chore.",
                {
                  resolvedHouseholdId,
                },
              );
            ...
            if (!name) {
              console.error("[CreateChores] Missing chore name/title.", {
                payload,
              });
              return;
            }
            ...
            if (!Array.isArray(rawAssignees) || rawAssignees.length === 0) {
              console.error("[CreateChores] No assignees were added", {
                payload,
              });
              return;
            }
            ...
            if (currentUserErr || !currentUser) {
              console.error(
                "[CreateChores] Could not get current user.",
                currentUserErr,
              );
              return;
            }
            ...
            if (otherNames.length > 0) {
              const { data: foundProfiles, error: profileLookupErr } =
                await supabase
                  .from("profiles")
              ...
              if (profileLookupErr) {
                console.error(
                  "[CreateChores] profiles lookup error:",
                  profileLookupErr,
                );
                return;
              }
            ...
            const nameToId = new Map(
              profileRows.map((p) => [p.display_name, p.id]),
            );
            ...
            const repeat_mask =
              typeof payload?.repeatBitmask === "number"
                ? payload.repeatBitmask
                : 0;
            ...
            if (beforeErr) {
              console.warn(
                "[CreateChores] Could not compute beforeCount.",
                beforeErr,
              );
            }
            ...
            if (assignErr) {
              console.error(
                "[CreateChores] assignment insert error:",
                assignErr,
                { rows },
              );
              return;
            ...
            if (afterErr) {
              console.warn(
                "[CreateChores] Could not compute afterCount.",
                afterErr,
              );
            }
            ...
            if (verifyErr) {
              console.warn(
                "[CreateChores] Could not verify chore_assignments rows.",
                verifyErr,
              );
            }
            ...
            if (sizeChanged || hasRowsForChore) {
              ...
              console.warn(
                "[CreateChores] chore_assignments did not change as expected.",
                {
                  beforeCount,
                  afterCount,
                  verifyRows,
            ...
            console.error("[CreateChores] onCreate fatal error:", e, {
              payload,
            });
          }
        }}
